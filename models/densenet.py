...
# Use Nvidia APEX DDP and DeepSeparableConv2d to speed up train and reduce GPU memory consumption
...
class Bottleneck(nn.Module):
    def __init__(self, nChannels, growthRate, drop_rate, BatchNorm, dsconv, training):
...
class SingleLayer(nn.Module):
    def __init__(self, nChannels, growthRate, BatchNorm, dsconv):
...
class DenseNet(nn.Module):
    def __init__(self, backbone, compression, num_classes, bottleneck, drop_rate, sync_bn, dsconv, training):
        ...
        self.dense1  = self._make_dense(nChannels, growthRate, nDenseBlocks1, bottleneck, drop_rate, BatchNorm, dsconv, training)
        ...
        self.dense2  = self._make_dense(nChannels, growthRate, nDenseBlocks2, bottleneck, drop_rate, BatchNorm, dsconv, training)
        nChannels   += nDenseBlocks2 * growthRate
        nOutChannels = int(math.floor(nChannels * compression))
        self.trans2  = Transition(nChannels, nOutChannels, drop_rate, BatchNorm, training)

        nChannels    = nOutChannels
        self.dense3  = self._make_dense(nChannels, growthRate, nDenseBlocks3, bottleneck, drop_rate, BatchNorm, dsconv, training)
        nChannels   += nDenseBlocks3 * growthRate
        nOutChannels = int(math.floor(nChannels * compression))
        self.trans3  = Transition(nChannels, nOutChannels, drop_rate, BatchNorm, training)

        nChannels   = nOutChannels
        self.dense4 = self._make_dense(nChannels, growthRate, nDenseBlocks4, bottleneck, drop_rate, BatchNorm, dsconv, training)
    ...
    def _make_dense(self, nChannels, growthRate, nDenseBlocks, bottleneck, drop_rate, BatchNorm, dsconv, training):
        layers = []
        for i in range(int(nDenseBlocks)):
            if bottleneck:
                layers.append(Bottleneck(nChannels, growthRate, drop_rate, BatchNorm, dsconv, training))
            else:
                layers.append(SingleLayer(nChannels, growthRate, BatchNorm, dsconv))
